...
export default class WinSWConfig {
	...
	_patch(data) {
		...
		// console.debug('[WinSWConfig#_patch]', data)
		for (const key in data) {
			...
			case 'entry':
				// Only splice entry
				this.arguments.splice(0, this.arguments.length, data[key]);
				break;
			case 'id':
				this[key] = data[key];
				break;
			case 'logOnAs':
				const logOnAs = data[key];
				this.serviceAccount.username = `${logOnAs.domain || (!logOnAs.account?.includes('\\') ? '.\\' : '')}${logOnAs.account}`;
				break;
			case 'runtimeExec':
		...
		if (data.options) this._patch(data.options)
	}
...
function getJavaScriptRuntimePath(allowed = ['bun', 'node']) {
	const execPath = process.execPath
		, baseName = basename(execPath);
	if (-1 !== allowed.findIndex(rt => baseName === `${rt}.exe`)) return execPath;
	try {
		return execSync('where bun || where node', { encoding: 'utf8' }).trim().split('\n').pop();
	} catch {
		return null
	}
}