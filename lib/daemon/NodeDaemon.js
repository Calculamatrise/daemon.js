...
export default class NodeDaemon extends Daemon {
	...
	get targetPath() {
		const path = this.winsw.config.arguments.at(-1);
	...
	set targetPath(value) {
		const path = this.winsw.config.arguments.at(-1);
		if (!isAbsolute(path)) {
			this.winsw.config.arguments.push(value);
		} else {
			this.winsw.config.arguments.splice(this.winsw.config.arguments, 1, value);
		}
	}
