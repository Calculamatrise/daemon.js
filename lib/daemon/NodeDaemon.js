import { basename, extname, isAbsolute, normalize, resolve } from 'path';
...
export default class NodeDaemon extends Daemon {
	...
	get entry() {
		...
		if (path != null) {
			...
			if (!ext.match(JS_EXT)) path = this.winsw.config.arguments.find(a => extname(a).match(JS_EXT));
		}
	...
	set entry(value) {
		if (!existsSync(value)) throw new ReferenceError(`Entry script not found at "${value}"`);
