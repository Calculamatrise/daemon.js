...
export default class Daemon extends DaemonInfo {
	...
	async update(info = null, password = null) {
		if (!await DaemonRegistry.isRegistered(this.id)) throw new ReferenceError(`Daemon "${this.id}" is not registered. Please use Daemon#install first.`);
		if (info) info = diff(this, info);
		await OSAdapter.config(this.id, info || this, password);
		if (info) this._patch(info)
	}
}

function diff(target, ...mutations) {
	const result = {};
