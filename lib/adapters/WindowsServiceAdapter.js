...
export default class WindowsServiceAdapter {
	...
	static async create(name, binary, info = null, password = null) {
		if (typeof password == 'string' || typeof password == 'number') info = Object.assign({}, info, { password });
		let paramsString = typeof info == 'object' ? formatParams(infoToCParamObj(info)) : '';
	...
	static async config(name, info) {
		let paramsString = typeof info == 'object' ? formatParams(Object.assign(infoToCParamObj(info), { binPath: info.binaryPath })) : '';
...
function infoToCParamObj(info) {
	...
	return {
		...
		password: info.logOnAs?.password,
