...
// Validate required
if (!config.file || !existsSync(resolve(config.file))) throw new Error(config.file + ' does not exist or cannot be found.');
if (!config.log) config.log = `${basename(config.file, extname(config.file))} Wrapper`;
